...
def upsert_heat_shelters(conn, rows: list[dict]):
    ...
    logger.info(
        f"[무더위] 신규: {new_count}건 | 변경: {changed_count}건 | 전체 UPSERT: {len(values)}건"
    )
...
def soft_delete_heat(conn, current_keys: list[tuple]):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT area_cd, shelter_name FROM public.heat_shelter WHERE is_deleted = false"
        )
...
def upsert_cold_shelters(conn, rows: list[dict]):
    ...
    logger.info(
        f"[한파] 사용여부 N 제외: {filtered}건 | 이용가능인원 0/NULL 제외: {skipped}건"
    )
    if not values:
    ...
    logger.info(
        f"[한파] 신규: {new_count}건 | 변경: {changed_count}건 | 전체 UPSERT: {len(values)}건"
    )
...
def soft_delete_cold(conn, current_keys: list[tuple]):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT no, shelter_name FROM public.cold_shelter WHERE is_deleted = false"
        )
        db_keys = set(tuple(r) for r in cur.fetchall())
...
def fetch_air_quality() -> dict:
    ...
    try:
        ...
        for row in rows:
            ...
            pm10 = (
                float(row["PM"])
                if str(row.get("PM", "")).replace(".", "", 1).isdigit()
                else 0.0
            )
            pm25 = (
                float(row["FPM"])
                if str(row.get("FPM", "")).replace(".", "", 1).isdigit()
                else 0.0
            )
    ...
    for dist in SEOUL_DISTRICTS:
        ...
        for attempt in range(max_retries):
            try:
                ...
                air = air_map.get(
                    dist["name"], {"pm10": 0.0, "pm25": 0.0, "grade": "데이터없음"}
                )
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"[환경] {dist['name']} 재시도 {attempt+1}/{max_retries}: {e}"
                    )
                    time.sleep(1)
                else:
                    logger.error(
                        f"[환경] {dist['name']} 수집 실패 (최대 재시도 초과): {e}"
                    )
